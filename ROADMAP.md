...
DUBSAR is moving from a developer-first private-beta narrative to a **portal-first audit and governance product for AI agents and business automations**.

The sequence is deliberate:

```text
clean and reconcile the foundation
  -> prove one internal audit journey
  -> open a controlled portal alpha
  -> validate with bounded external use
  -> add continuous governance through DUBSAR Node
  -> expand connectors and optional host adapters
```

No external agency pilot should replace the internal end-to-end proof.
...
## 2. Internal portal end-to-end proof

The first complete product proof must be performed internally with non-sensitive fixtures.
...
## 3. Controlled portal alpha

Only after the internal end-to-end proof may DUBSAR open a limited portal alpha.
...
The alpha should not require a manual accompanied audit for every user. Human assistance remains optional for qualification, interpretation or a broader mandate.
...
## 8. Developer-tool adapters

Claude Code, Codex and Cursor integrations remain a parallel experimental path.
...
These integrations should reuse the same private Core and authority model. They must not become separate product brains or delay the portal-first audit.

No public adapter availability is currently claimed.
...
- supported Claude Code, Codex or Cursor products.
...
controlled portal alpha: not yet opened
external agency pilot: not started
continuous governance / DUBSAR Node: future stage
